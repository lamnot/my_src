/*
	Library for the Olimex MOD-SMB380 board + STM32 boards
	...
	Tested with Olimexino-STM32 and MOD-SMB380
	
	|-------------------
	| v1.0 - July 2012, Maple IDE version 0012
	|-------------------
...
{
private:
	volatile uint16 accX, accY, accZ;
...
public:
   ...
   /*** Class Interface ***/
	 ...
	 **/
	void WriteOneAddr(unsigned char W_ADDR, uint8 data){
	 ...
	 * 	  device on the same CSD and SDI line.
	 **/	
	void ReadAddr(unsigned char R_ADDR, uint8 * buffer, uint8 NumOfBytes){
	 ...
	 *	
	 **/
	uint8 ReadOneAddr(unsigned char R_ADDR){
		unsigned char res=0;
		char i=0;
	 ...
	 **/
	void updateData(){
		static uint16 rawX, rawY, rawZ, rawT;
		static uint8  buf[6];
		static char lsb;
	 ...
	 **/	 
	uint16 getAccX(){ return accX;}
	
	
	/** 
	 *	Returns the Y acceleration data (10 bits) 
	 **/
	uint16 getAccY(){ return accY;}
	
	
	/** 
	 *	Returns the Z acceleration data (10 bits) 
	 **/
	uint16 getAccZ(){ return accZ;}
	 ...
	 *			using references
	 **/
	void getAcc( uint16 &X, uint16 &Y, uint16 &Z){
		X = getAccX();
		Y = getAccY();
		Z = getAccZ();
	}
