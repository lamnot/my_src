...
void DhcpClass::send_DHCP_MESSAGE(uint8_t messageType, uint16_t secondsElapsed)
{
    ...
    if(messageType == DHCP_REQUEST)
    {
        ...
        buffer[7] = 0x04;
        buffer[8] = ((uint8_t*)&_dhcpDhcpServerIp)[0];
        buffer[9] = ((uint8_t*)&_dhcpDhcpServerIp)[1];
        buffer[10] = ((uint8_t*)&_dhcpDhcpServerIp)[2];
        buffer[11] = ((uint8_t*)&_dhcpDhcpServerIp)[3];
        ...
        while (_dhcpUdpSocket.available() > 0) 
        {
            switch (_dhcpUdpSocket.read()) 
            {
                ...
                case dhcpServerIdentifier :
                    opt_len = _dhcpUdpSocket.read();
                    if( _dhcpDhcpServerIp == 0 || 
                        IPAddress((uint8_t*)&_dhcpDhcpServerIp) == _dhcpUdpSocket.remoteIP() )
                    {
                        _dhcpUdpSocket.read((uint8_t*)&_dhcpDhcpServerIp, sizeof(_dhcpDhcpServerIp));
                    }
                    else
                    {
...
{
    return IPAddress((uint8_t*)&_dhcpDhcpServerIp);
}
